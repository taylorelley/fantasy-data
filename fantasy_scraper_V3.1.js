...
async function main() {
  ...
  try {
    ...
    await page.goto(CONFIG.DRIVER_URL, {
    ...
    await page.goto(CONFIG.CONSTRUCTOR_URL, {
      waitUntil: "networkidle",
      timeout: 60000,
    });
    await page.waitForSelector(".si-main__container", { timeout: 30000 });
    ...
    console.log(
      `✅ Found ${constructorElements.length} constructors in main list`,
    );
    ...
    console.log(`\n🎉 SCRAPING COMPLETE!`);
    console.log(
      `📊 Successfully processed ${driverBreakdowns.size} unique drivers`,
    );
    console.log(
      `🏗️  Successfully processed ${constructorBreakdowns.size} unique constructors`,
    );
    console.log(
      `🔄 Team swap drivers handled: ${Array.from(driverBreakdowns.values()).filter((d) => d.teamSwap).length}`,
    );
    ...
    console.log(
      `📁 Individual drivers: ${versionFolder}/driver_data/ & latest/driver_data/`,
    );
    console.log(
      `📁 Individual constructors: ${versionFolder}/constructor_data/ & latest/constructor_data/`,
    );
    console.log(
      `📁 Summary data: ${versionFolder}/summary_data/ & latest/summary_data/`,
    );
  } catch (error) {
...
async function saveResults() {
  ...
  console.log(
    "🔍 Keys should now be in correct order:",
    Object.keys(sortedConstructorSummary).slice(0, 5),
  );

  const constructorSummaryJson = JSON.stringify(
  ...
  const detailedSummary = {
    ...
    totalRaces: Array.from(driverBreakdowns.values()).reduce(
      (sum, driver) => sum + driver.races.length,
      0,
    ),
    averagePercentagePicked:
      driverBreakdowns.size > 0
        ? Math.round(
            Array.from(driverBreakdowns.values()).reduce(
              (sum, d) => sum + d.percentagePicked,
              0,
            ) / driverBreakdowns.size,
          )
        : 0,
    averageConstructorPercentagePicked:
